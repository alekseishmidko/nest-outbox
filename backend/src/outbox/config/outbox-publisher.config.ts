...
const outboxPublisherEnvSchema = z.object({
  ...
  OUTBOX_RETRY_MAX_DELAY_MS: z.coerce.number().int().positive().default(60_000),
  OUTBOX_RETRY_JITTER_MS: z.coerce.number().int().min(0).default(0),
  OUTBOX_SHUTDOWN_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(10_000),
